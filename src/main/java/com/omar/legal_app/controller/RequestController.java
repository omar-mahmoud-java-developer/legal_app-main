...
@GetMapping("/list")
    public String showRequestList(Model model){
        ...
        List<RequestEntity> requestEntitys = requestRepo.findAll();
        model.addAttribute("request", requestEntitys);
        return "om";
    }
    @GetMapping("/create")
    ...
    @PostMapping("/create")
    public String createRequest(@Valid @ModelAttribute ReuqesrDto reuqesrDto, BindingResult result, Model model) {
    ...
    @GetMapping("/delete")
    public String deletRequrst(

@RequestParam int id ){

    try {
        RequestEntity rEntity =requestRepo.findById(id).get();
